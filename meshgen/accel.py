...
def get_device():
    """
    Get the device to use for computations (always returns None for CPU-only)
    
    Returns:
        None: Always returns None since we're CPU-only
    """
    return None
...
def is_gpu_available() -> bool:
    """
    Check if GPU acceleration is available (always False for CPU-only)
    
    Returns:
        bool: Always False since we're CPU-only
    """
    return False
...
def array_operations(
    vertices: np.ndarray,
    operations: str = 'radius'
) -> np.ndarray:
    """
    CPU-based array operations for vertex processing
    
    Args:
        vertices: Vertex positions (N, 3)
        operations: Type of operation ('radius', 'identify_outer', etc.)
        
    Returns:
        np.ndarray: Result of the operation
    """
    if operations == 'radius':
        return np.sqrt(vertices[:, 0]**2 + vertices[:, 1]**2)
    elif operations == 'identify_outer':
